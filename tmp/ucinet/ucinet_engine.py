...
class UciNetServerNamedPipe:
    ...
    def __init__(self):
        self._proto_in_path = os.path.join(__dirname__, "proto_in")
        self._proto_out_path = os.path.join(__dirname__, "proto_out")
        self._proto_out = open(self._proto_out_path, "wb", buffering=0)
    ...
    def serve(self, process_cmd_func: Callable[[str], str]) -> None:
        ...
        try:
            while True:
                ...
                try:
                    with open(self._proto_in_path, "rb", buffering=0) as proto_in:
                        while True:
                    ...
                    print(f"[ucinet_engine] FIFO '{self._proto_in_path}' not found, retrying...", file=sys.stderr)
                    time.sleep(1.0)
                    continue
