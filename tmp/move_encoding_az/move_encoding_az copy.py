...
# You already have these defined:
# SLIDING_DIRS, KNIGHT_DIRS, PROMOTION_PIECES, PROMOTION_DIRS
...
def describe_plane(from_sq: int, plane: int, board: chess.Board):
    ...
    """
    decoded_move = decode_move_from_plane(from_sq, plane, board)
    ...
    if 0 <= plane <= 55:
        dir_index = plane // 7
        distance = (plane % 7) + 1
        direction = DIRECTION_NAMES[dir_index]
        desc = f"Slide {direction} by {distance} from {from_name}"

    # Knight moves:
    elif 56 <= plane <= 63:
        knight_index = plane - 56
        delta_desc = KNIGHT_NAMES[knight_index]
    ...
    # Underpromotion:
        ...
        direction_name = PROMO_DIR_NAMES[dir_index]
        promo_piece = PROMO_PIECE_NAMES[PROMOTION_PIECES[promo_index]]
...
def move_to_plane(move: chess.Move, board: chess.Board) -> tuple[int, int] | None:
    ...
    # 1. Sliding moves
    for dir_index, (sx, sy) in enumerate(SLIDING_DIRS):
    ...
    # 2. Knight moves
    for knight_index, (kx, ky) in enumerate(KNIGHT_DIRS):
        if (dx == kx) and (dy == ky):
    ...
    if move.promotion in PROMOTION_PIECES:
        ...
        rel_dy = ty - fy
        if rel_dx == px_dir:
            for dir_idx, (px, py) in enumerate(PROMOTION_DIRS):
...
def encode_move_tensor(move: chess.Move, board: chess.Board) -> torch.Tensor:
    ...
    result = move_to_plane(move, board)
    if result is None:
...
def decode_move_from_plane(from_sq: int, plane: int, board: chess.Board):
    ...
    if 56 <= plane <= 63:
        knight_index = plane - 56
        kx, ky = KNIGHT_DIRS[knight_index]
        tx = fx + kx
        ty = fy + ky
    ...
    if 64 <= plane <= 72:
        ...
        sx, sy = PROMOTION_DIRS[dir_index]
        tx = fx + sx * px_dir  # Only forward by 1
        ty = fy + sy

        if 0 <= tx < 8 and 0 <= ty < 8:
            to_sq = tx * 8 + ty
            promo_piece = PROMOTION_PIECES[promo_index]
...
if __name__ == "__main__":
    ...
    single_move_tensor = encode_move_tensor(move, board)
    legal_mask = encode_legal_moves_mask(board)
    ...
    # Decode
    decoded = decode_move_from_plane(fsq, plane, board)
    assert decoded != None, "Decoding failed"
    print("Decoded:", decoded, "→", decoded.uci())

    decoded_move, explanation = describe_plane(fsq, plane, board)
