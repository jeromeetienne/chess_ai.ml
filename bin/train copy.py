...
class TrainCommand:
    ...
    def train(
        ...
        # NOTE: why is this needed on evals_tensor but not moves_tensor... because
        # - evals is a regression and use SmoothL1Loss as loss function. SmoothL1Loss expects targets of shape (N, 1)
        # - moves is a classification problem and uses CrossEntropyLoss. CrossEntropyLoss expects targets of shape (N,) with class indices
        ...
        for epoch_index in range(max_epoch_count):
            ...
            valid_loss, valid_cls_loss, valid_reg_loss = TrainCommand.validate_one_epoch(
    ...
    def evaluate_model(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, device: str) -> tuple[float, float]:
        ...
        with torch.no_grad():
            for boards_tensor, moves_tensor, evals_tensor in dataloader:
                ...
                _, move_predictions = torch.max(move_logits, 1)  # Get the index of the max log-probability
                move_accuracy_count += moves_tensor.size(0)
                move_accuracy_correct += (move_predictions == moves_tensor).sum().item()

                # Compute regression MAE using SmoothL1Loss directly
                smooth_l1_loss = torch.nn.SmoothL1Loss(reduction="sum")
                eval_mae_count += evals_tensor.size(0)
                eval_mae_sum += smooth_l1_loss(eval_pred, evals_tensor).item()
        ...
        return move_mean_accuracy, eval_mean_mae
