...
class WeightInitializer:
    ...
    def init_weights(model: nn.Module) -> None:
        ...
        * **LSTM Forget Gate (Bias = 1)**: This is a very common trick. By setting the forget gate's bias to a positive value (like 1), the gate's output
        ...
        model.apply(WeightInitializer._init_weights)

    @staticmethod
    def _init_weights(model: nn.Module) -> None:
        if isinstance(model, (nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.Linear)):
        ...
        elif isinstance(model, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.GroupNorm)):
        ...
        elif isinstance(model, (nn.RNN, nn.LSTM, nn.GRU)):
