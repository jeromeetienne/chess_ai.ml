...
def test_input_shape():
    ...
    assert BoardEncoding.get_input_shape() == (BoardEncoding.PLANE.PLANE_COUNT, 8, 8)


def test_starting_position_planes_and_metadata():
    board = chess.Board()  # standard starting position
    tensor = BoardEncoding.board_to_tensor(board)

    # shape and dtype
    assert tuple(tensor.shape) == BoardEncoding.get_input_shape()
    assert tensor.dtype == BoardEncoding.BOARD_DTYPE

    # piece counts: each side has 8 pawns, 2 knights, 2 bishops, 2 rooks, 1 queen, 1 king
    # active (white) planes indices 0-5
    pawn_plane = tensor[BoardEncoding.PLANE.ACTIVE_PAWN]
    assert int(pawn_plane.sum()) == 8

    knight_plane = tensor[BoardEncoding.PLANE.ACTIVE_KNIGHT]
    assert int(knight_plane.sum()) == 2

    bishop_plane = tensor[BoardEncoding.PLANE.ACTIVE_BISHOP]
    assert int(bishop_plane.sum()) == 2

    rook_plane = tensor[BoardEncoding.PLANE.ACTIVE_ROOK]
    assert int(rook_plane.sum()) == 2

    queen_plane = tensor[BoardEncoding.PLANE.ACTIVE_QUEEN]
    assert int(queen_plane.sum()) == 1

    king_plane = tensor[BoardEncoding.PLANE.ACTIVE_KING]
    assert int(king_plane.sum()) == 1

    # opponent (black) planes indices 6-11
    opp_pawn_plane = tensor[BoardEncoding.PLANE.OPPONENT_PAWN]
    assert int(opp_pawn_plane.sum()) == 8

    # metadata: white to move, fullmove_number=1, halfmove_clock=0
    assert int(tensor[BoardEncoding.PLANE.TURN, 0, 0].item()) == 1
    assert int(tensor[BoardEncoding.PLANE.FULLMOVE_NUMBER, 0, 0].item()) == 1
    assert int(tensor[BoardEncoding.PLANE.HALFMOVE_CLOCK, 0, 0].item()) == 0


def test_roundtrip_reconstruction_white():
    board = chess.Board()
    tensor = BoardEncoding.board_to_tensor(board)
    reconstructed = BoardEncoding.board_from_tensor(tensor)
...
def test_roundtrip_reconstruction_black_perspective():
    ...
    assert int(tensor[BoardEncoding.PLANE.TURN, 0, 0].item()) == 0
...
def test_cycle_idempotence_multiple_times():
    ...
    for _ in range(5):
        tensor = BoardEncoding.board_to_tensor(board)
        board = BoardEncoding.board_from_tensor(tensor)
